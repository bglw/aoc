...
def get_space_deficit(fs: FS, total=70000000, needed=30000000):
    return needed - (total - calc(fs))


def find_smallest_largest_dir(fs: FS, largerThan: int):
    options = []
    for dir in fs.dirs.values():
        size = calc(dir)
        if size > largerThan:
            options.append(size)
        options.extend(find_smallest_largest_dir(dir, largerThan))
    return options


def run(input: str):
    fs = build_fs(input)
    dirs = find_smallest_largest_dir(fs, get_space_deficit(fs))
    dirs.sort()
    return dirs[0]
...
class Tests:
    def test_run(self):
        assert run(test_data) == 24933642

    def test_size(self):
        fs = build_fs(test_data)
        assert get_space_deficit(fs) == 8381165
